...
const HotelCard = ({room, index}) => {
     if (!room || !room.hotel) return null; // ✅ prevent crash
  return (
        ...
        <div className='p-4 pt-5'>
            ...
            <div>
             <div className='flex item-center gap-1 text-sm'>
                  ...
                  <span>{room.hotel?.address  || "Location unavailable"}</span>
             </div>
            </div>
                <div className='flex item-center justify-between mt-4 '>
                    <p><span className='text-xl text-gray-800'>${room?.pricePerNight}/night</span></p>
                       <button className='px-4 py-2 text-sm font-medium border border-gray-300 rounded hover:bg-gray-50 transition-all cursor-ponter'>Book Now</button>
                </div>
            </div>
            </Link>
